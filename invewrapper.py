...
source_inve = os.path.normpath(os.path.join(os.path.dirname(__file__), '../../../../bin/inve'))
...
def mkvirtualenv_cmd():
	...
	parser.add_argument('-i', action='append', dest='packages', help='Install a package after the environment is created. This option may be repeated.')
	parser.add_argument('-a', dest='project', help='Provide a full path to a project directory to associate with the new environment.')
	parser.add_argument('-r', dest='requirements', help='Provide a pip requirements file to install a base set of packages into the new environment.')
	...
	with chdir(workon_home):
		...
		with chdir(args.envname):
			...
			# temporary workaround: as soon as virtualenv's PR #247 will be completed I'll remove it
...
def workon_cmd():
	...
	if not os.path.exists(env_path):
		print("ERROR: Environment '{0}' does not exist. Create it with 'mkvirtualenv {0}'.".format(env), file=sys.stderr)
		return
	else:
		inve = get_inve(env)
		check_call([inve])
