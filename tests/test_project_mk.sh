...
test_create_directories () {
    echo "" | pew_mkproject myproject1 >/dev/null 2>&1
...
test_create_virtualenv () {
    echo "" | pew_mkproject myproject2 >/dev/null 2>&1
	venv=$(echo "echo \$VIRTUAL_ENV" | pew_workon myproject2 | tail -n1)
...
test_no_project_home () {
    ...
    output=`pew_mkproject should_not_be_created 2>&1`
    assertTrue "Did not see expected message" "echo $output | grep 'does not exist'"
    PROJECT_HOME="$old_home"
}

test_project_exists () {
    echo "" | pew_mkproject myproject4 >/dev/null 2>&1
    output=`pew_mkproject myproject4 2>&1`
    assertTrue "Did not see expected message 'already exists' in: $output" "echo $output | grep 'already exists'"
}

test_same_pew_workon_and_project_home () {
    old_project_home="$PROJECT_HOME"
    export PROJECT_HOME="$WORKON_HOME"
    echo "" | pew_mkproject myproject5 >/dev/null 2>&1
    assertTrue "env directory not created" "[ -d $WORKON_HOME/myproject1 ]"
