...
test_activates () {
    ...
    assertSame "env2" $(basename $(echo "echo \$VIRTUAL_ENV" | pew-new "env2" | tail -n1))
}
...
test_no_virtualenv () {
    ...
    pew-new should_not_be_created 2>/dev/null
    RC=$?
...
test_no_args () {
    pew-new 2>/dev/null 1>&2
    RC=$?
    assertSame "2" "$RC"
}


test_pew-new_sitepackages () {
	...
	echo "" | pew-new "with_sp" --system-site-packages >/dev/null 2>&1
    ngsp_file=$(echo "pew-sitepackages_dir" | pew-workon with_sp | tail -n1 )"/../no-global-site-packages.txt"
    assertFalse "$ngsp_file exists" "[ -f \"$ngsp_file\" ]"
    
    # With the argument, verify that they are not copied.
    echo "" | pew-new "without_sp" --no-site-packages >/dev/null
    ngsp_file=$(echo "pew-sitepackages_dir" | pew-workon without_sp | tail -n1 )"/../no-global-site-packages.txt"
