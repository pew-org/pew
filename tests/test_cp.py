...
def test_new_env_activated(workon_home, testpackageenv):
    ...
    assert ('destination', 'testscript.py') == (
        testscript.parts[-3], testscript.parts[-1])
...
def test_virtualenv_variable(copied_env):
    envname = invoke(
        'workon', copied_env.name, inp='echo $VIRTUAL_ENV'
    ).out.strip()
...
def test_no_global_site_packages(copied_env):
    site = Path(
        invoke('workon', copied_env.name, inp='pew sitepackages_dir').out
    )
